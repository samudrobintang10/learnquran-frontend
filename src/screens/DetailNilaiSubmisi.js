...
export default function DetailNilaiSubmisi({ navigation, route }) {
  ...
  const getDetailTask = async (id, id_student) => {
    ...
    try {
      ...
      setNilai(response?.results?.data?.submission?.score);
      setFeedback(response?.results?.data?.submission?.feedback);
          ...
          <Card.Content>
            ...
            <ScrollView>
              <View style={styles.row}>
                <Text style={styles.outerText}>Nilai : </Text>
                <Gap width={15} />
                <Text style={styles.grade}>{nilai}</Text>
              </View>
              <Gap height={15} />
